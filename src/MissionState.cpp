...
MissionState::MissionState(App &app) : app(app) {
    ...
    m_helpMonitorSurface = SDL_CreateRGBSurface(
        0, m_helpMonitorImage->w, m_helpMonitorImage->h,
        m_helpMonitorImage->format->BitsPerPixel,
        m_helpMonitorImage->format->Rmask, m_helpMonitorImage->format->Gmask,
        m_helpMonitorImage->format->Bmask, m_helpMonitorImage->format->Amask);

    SDL_BlitSurface(m_helpMonitorImage, nullptr, m_helpMonitorSurface, nullptr);

    m_helpMonitor = std::make_shared<TexturedRect2D>(
        HELPER_X_POSITION, -WIDTH_RATIO / 2, HELPER_WIDTH, WIDTH_RATIO);
    ...
    m_currentMission =
        std::make_unique<TutorialMission>(app, *this, m_vimEmulator);
